...
module mod_solu
  ...
  real,pointer::Gauss_weight(:)             !Gauss_weight(np)
  !**********solutions related******************************!
  real,pointer::QC(:,:),Res(:,:)                      !QC,Res(np,nc)
  real,pointer::Flux(:,:)                             !Flux(np,nc)  
  real,pointer::rki(:,:),pki(:,:),uki(:,:),tki(:,:),eki(:,:)                      !rho, u, p, t(np,nc)
  real,pointer::r_m(:),p_m(:),u_m(:),t_m(:)
  ...
  !**********RK related*******************************!
  integer,parameter::kcmax=100000
  real::delt=0.01
  real,parameter::kstage=3
  integer,parameter::lim_av=1

  real::tprint  !end of time
  real::t       !current time
end module mod_solu
...
program CPRoneDBGK
  ...
  grid%np = np
  grid%nf = nf
  allocate(x(nf),xc(np,nc),xcc(nc))
  allocate(Coe_grid(np),Coe_RadauL(np),Coe_RadauR(np),Coe_DLag(np,np),Coe_Lag(np,2))
  allocate(QC(np,nc),Res(np,nc))
  allocate(Flux(np,nc))
  allocate(Gauss_weight(np))
  allocate(r_m(nc),p_m(nc),u_m(nc),t_m(nc))
  if(lim_av>0) allocate(mark(nc))

  nv=100
  allocate(gh(nv),ww(nv),vv(nv),cc(nv))
  allocate(rki(np,nc),pki(np,nc),uki(np,nc),tki(np,nc),eki(np,nc))
  allocate(feq(np,nc,nv))
  ...
  if(np==2) then                     !Guass-Lergendre points plus two ends
     ...
     Gauss_weight(1) = (18.-sqrt(30.))/36.
     Gauss_weight(2) = (18.+sqrt(30.))/36.
     Gauss_weight(3) = Gauss_weight(2)
     Gauss_weight(4) = Gauss_weight(1)
  else if(np==5) then
     Coe_grid(1)=-sqrt(5.+2.*sqrt(10./7.))/3.
     Coe_grid(2)=-sqrt(5.-2.*sqrt(10./7.))/3.
     Coe_grid(3)=0.0
     Coe_grid(4)=sqrt(5.-2.*sqrt(10./7.))/3.
     Coe_grid(5)=sqrt(5.+2.*sqrt(10./7.))/3.
     Gauss_weight(1) = (322.-13.*sqrt(70.))/900.
     Gauss_weight(2) = (322.+13.*sqrt(70.))/900.
     Gauss_weight(3) = 128./225.
     Gauss_weight(4) = Gauss_weight(2)
     Gauss_weight(5) = Gauss_weight(1)
  end if
  ...
  do
     ...
     do iv=1,nv
        ...
        write(10,*) kcount,ResSum
        write(*,*) 'iv=',iv,'res=',ResSum 
        do ic=1,nc
           do j=1,np
              feq(j,ic,iv) = QC(j,ic) 
           end do
     ...
     call macrov(grid)
     print *,'iter=',kcount,'time=',t,'dt=',delt
...
301 format(e12.5,1x,e12.5,1x,e12.5,1x,e12.5,1x,e12.5,1x)              
  ...
  open(unit=10,file='solution_ave.dat')
  do i=1,nc
     call icell_ave(r_m(i),rki(1,i),np,i)
     call icell_ave(p_m(i),pki(1,i),np,i)
     call icell_ave(u_m(i),uki(1,i),np,i)
     call icell_ave(t_m(i),tki(1,i),np,i)
     write(10,301) xcc(i),r_m(i),p_m(i),u_m(i),t_m(i)
  end do
  close(10)
  stop  
...
!*********************************************************************************************************************!
subroutine initial(grid)
  use mod_solu
  implicit none
  type(tgrid)::grid
  integer::iv,ic,nc,np,j
  real::v1,v2,dv,rl,ul,ur,pl,pr,rr,pres
...
!!-------------------------------------------
subroutine macrov(grid)
  use mod_solu
  implicit none

  type(tgrid)::grid
  integer::ic,nc,np,j,iv
  real::skr,sku,ske,pres
  real,pointer::rki_m(:),uki_m(:),tki_m(:)
  ...
  if(lim_av>0) then
  !------- simply enforce mean macro value ------!
    allocate(rki_m(nc),uki_m(nc),tki_m(nc))

    do ic = 1, nc
       call icell_ave(rki_m(ic),rki(1,ic),np,ic)
       call icell_ave(uki_m(ic),uki(1,ic),np,ic)
       call icell_ave(tki_m(ic),tki(1,ic),np,ic)
    end do
 
    mark = 0
    !----- simply find the overshoot cells -----!
    do ic = 2, nc-1
       if( (rki_m(ic) > rki_m(ic-1) .and. rki_m(ic) > rki_m(ic+1)) &
      .or. (rki_m(ic) < rki_m(ic-1) .and. rki_m(ic) < rki_m(ic+1)) ) then

         if(abs(rki_m(ic)-rki_m(ic-1))<1.e-8 .and. abs(rki_m(ic)-rki_m(ic+1))<1.e-8)  mark(ic) = 1
       end if
    end do   
    ...
    do iv = 1, nv
       do ic = 1, nc
          if(mark(ic)==1) then
            pres  = (vv(iv)-uki_m(ic))**2/tki_m(ic)
            feq(1:np,ic,iv) = rki_m(ic)*exp(-pres)/sqrt(pi*tki_m(ic))
          else
            do j=1,np
               pres  = (vv(iv)-uki(j,ic))**2/tki(j,ic)
               feq(j,ic,iv) = rki(j,ic)*exp(-pres)/sqrt(pi*tki(j,ic))
...
!!
!!-------------------------------------------
subroutine setdelt(grid)
  use mod_solu
  implicit none

  type(tgrid)::grid
  real::cflc,dxmin,amax,tmt,rr
  integer::iv,np
...
subroutine FluxRec(grid)
  ...
  do ic=2,nc-1    !update CVs using correction via flux reconstructions
     do i=1,np
...
!***********************cell average***************!
subroutine icell_ave(qout,qin,np,icell)
  use mod_solu
  implicit none
  ...
  qout = 0.
  do i=1,np
     qout = qout + Gauss_weight(i)*qin(i)
  end do
  qout = qout*0.5
  
end subroutine icell_ave
