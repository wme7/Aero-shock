...
c coded by Qiu Jianxian  May, 2001         
     ...
     &,wf(2800,3,4),w(3),s(3),wab(2800,3),cf(3,3),uexact(2800)
...
c initial condition of cell averages
      ...
      u(i)=(u0(x)-u0(x-dx))/dx
      enddo
      call exact(n,uexact,tf,md)
c time evoluation loop

      do nn=1,1000000											
      xxx1=0.0
	do i=md+1,n+md+1
	xxx1=max(xxx1,abs(u(i)))
	enddo
      dt=cfl*dx/xxx1
...
c compt nonlinear weight for f     
	...
	s(1)=(ff(i-2,l_rk)-2.0*ff(i-1,l_rk)+ff(i,l_rk))**2
      s(2)=(ff(i-1,l_rk)-2.0*ff(i,l_rk)+ff(1+i,l_rk))**2
      s(3)=(ff(i,l_rk)-2.0*ff(i+1,l_rk)+ff(i+2,l_rk))**2
...
c v(i)=point value of u at xi at tn
	  v(i)=ttt   
      enddo
      yc='bur'// char(kkkk+48) //'.plt'
      open(1,file=yc)		
      do kk0=md,md+n
      write(1,*) dx*(kk0-md),v(kk0)
      enddo
      close(1)
	 error1=0.0
      error2=0.0
      do kk0=md+1,md+n 
      error1=error1+abs(uexact(kk0)-v(kk0))
      error2=max(error2,abs(uexact(kk0)-v(kk0)))
      enddo
      error1=error1*dx
...
c      fu1=(sin(pi*x))**4   
	...
	return
	  ...
	  subroutine exact(n,uexact,t,md)
        parameter(nn1=2800)    
      dimension xx(nn1),uu(nn1),uexact(nn1)
	open(1,file='B_exact1.dat')
	pi=4.0*atan(1.0)
	nn=n+2*md
	do i=1,nn
	xx(i)=2.0/n*(i-md)
	uu(i)=0.5+sin(pi*xx(i))
	enddo
      do 1 i=1,n+1
      xeee=(i-1)*2.*pi/n
      xee=xeee/pi
