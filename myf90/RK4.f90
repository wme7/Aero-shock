...
isolver		= 0		! euler = 0, navier-stokes = 1
...
call iteration (nv,nx,ny,dtdx,dtdy,dt,dx,dy,r_time,feq,v,f)
...
!
subroutine iteration (nv,nx,ny,dtdx,dtdy,dt,dx,dy,r_time,feq,v,f)
subroutine iteration (nv,nx,ny,dtdx,dtdy,dt,dx,dy,r_time,feq,v,f)
implicit none
integer igh,imaxgridx,imaxgridy
parameter (igh=20,imaxgridx=400,imaxgridy=400)
integer k,l,i,j,nv,nx,ny
real(8)	vxp,vxm,vyp,vym,dtdx,dtdy,dt,dx,dy,r_time
real,dimension (igh) :: v
real,dimension (imaxgridx,imaxgridy) ::fn,fl,fh,gl,gh2,H1,H2,H3,H4
real,dimension (igh,igh,imaxgridx,imaxgridy) :: f,f2,f3,f4,feq
do k = 1, nv
  do l = 1, nv
    vxp = max(v(k),0.)
    vxm = min(v(k),0.) 
	...
	! H(1)
	call stepone (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H1)
!	call implicit_mtd (nx,ny,k,l,vxp,vyp,vxm,vym,fl,fh,gl,gh2,dt,dtdx,dtdy,f,fn)
	do i = 2,nx-1
	  do j = 2,ny-1
		f2(k,l,i,j) = fn(i,j)
	    call b_cond (k,l,i,j,nx,ny,fn,f2)
	  end do
	end do	
	call tvdfluxes (nx,ny,k,l,vxp,vxm,vyp,vym,dtdx,dtdy,v,f2,fl,fh,gl,gh2)
	!H(2)
	call steptwo (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H2)
	do i = 2,nx-1
	  do j = 2,ny-1
		f3(k,l,i,j) = fn(i,j)
	    call b_cond (k,l,i,j,nx,ny,fn,f3)
	  end do8
	end do
	call tvdfluxes (nx,ny,k,l,vxp,vxm,vyp,vym,dtdx,dtdy,v,f3,fl,fh,gl,gh2)
	!H(3)
	call stepthree (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H3)
	do i = 2,nx-1
	  do j = 2,ny-1
		f4(k,l,i,j) = fn(i,j)
	    call b_cond (k,l,i,j,nx,ny,fn,f4)
	  end do
	end do
	call tvdfluxes (nx,ny,k,l,vxp,vxm,vyp,vym,dtdx,dtdy,v,f4,fl,fh,gl,gh2)
	!H(4)
	call stepfour (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H1,H2,H3,H4)
	do i = 2,nx-1
	  do j = 2,ny-1
		f(k,l,i,j) = fn(i,j)
...
! 
subroutine stepone (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H1)
implicit none		 
integer igh,imaxgridx,imaxgridy
parameter (igh=20,imaxgridx=400,imaxgridy=400)
integer k,l,i,j,nx,ny
real(8) dt,dtdx,dtdy,r_time,dx,dy
real, dimension (imaxgridx,imaxgridy) :: fl,fh,gl,gh2,fn,H1
real, dimension (igh,igh,imaxgridx,imaxgridy) :: f,feq
 do i = 2, nx-1
    do j = 2, ny-1
		H1(i,j) =  (- (fh(i,j)-fl(i,j))/dx - (gh2(i,j)-gl(i,j))/dy + (feq(k,l,i,j)-f(k,l,i,j))/r_time )
		fn(i,j) = f(k,l,i,j) + 0.5d0*dt*H1(i,j)
    end do
 end do
return
end subroutine
!
subroutine steptwo (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H2)
implicit none		 
integer igh,imaxgridx,imaxgridy
parameter (igh=20,imaxgridx=400,imaxgridy=400)
integer k,l,i,j,nx,ny
real(8) dt,dtdx,dtdy,r_time,dx,dy
real, dimension (imaxgridx,imaxgridy) :: fl,fh,gl,gh2,fn,H2
real, dimension (igh,igh,imaxgridx,imaxgridy) :: f,feq
 do i = 2, nx-1
    do j = 2, ny-1
		H2(i,j) =  (- (fh(i,j)-fl(i,j))/dx - (gh2(i,j)-gl(i,j))/dy + (feq(k,l,i,j)-f(k,l,i,j))/r_time )
		fn(i,j) = f(k,l,i,j) + 0.5d0*dt*H2(i,j)
    end do
 end do
return
end subroutine
!
subroutine stepthree (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H3)
implicit none		 
integer igh,imaxgridx,imaxgridy
parameter (igh=20,imaxgridx=400,imaxgridy=400)
integer k,l,i,j,nx,ny
real(8) dt,dtdx,dtdy,r_time,dx,dy
real, dimension (imaxgridx,imaxgridy) :: fl,fh,gl,gh2,fn,H3
real, dimension (igh,igh,imaxgridx,imaxgridy) :: f,feq
 do i = 2, nx-1
    do j = 2, ny-1
		H3(i,j) =  - (fh(i,j)-fl(i,j))/dx - (gh2(i,j)-gl(i,j))/dy + (feq(k,l,i,j)-f(k,l,i,j))/r_time 
		fn(i,j) = f(k,l,i,j) + dt*H3(i,j)
    end do
 end do
return
end subroutine
!
subroutine stepfour (k,l,nx,ny,fl,fh,gl,gh2,dt,dx,dy,dtdx,dtdy,feq,f,r_time,fn,H1,H2,H3,H4)
implicit none		 
integer igh,imaxgridx,imaxgridy
parameter (igh=20,imaxgridx=400,imaxgridy=400)
integer k,l,i,j,nx,ny
real(8) dt,dtdx,dtdy,r_time,dx,dy
real, dimension (imaxgridx,imaxgridy) :: fl,fh,gl,gh2,fn,H1,H2,H3,H4
real, dimension (igh,igh,imaxgridx,imaxgridy) :: f,feq
 do i = 2, nx-1
    do j = 2, ny-1
		H4(i,j) =  - (fh(i,j)-fl(i,j))/dx - (gh2(i,j)-gl(i,j))/dy +(feq(k,l,i,j)-f(k,l,i,j))/r_time 
		fn(i,j) = f(k,l,i,j) + (dt/6.0)*(H1(i,j)+(2*H2(i,j))+(2*H3(i,j))+H4(i,j))
    end do
 end do
return
end subroutine
