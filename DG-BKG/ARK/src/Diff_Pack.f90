...
subroutine DG2D_flux_erk(l)
  ...
  real(kind=8),allocatable :: temp3(:,:),tempsum3(:,:),temp4(:,:),tempsum4(:,:)
!  real(kind=8),allocatable :: tempb1(:,:),tempb2(:,:)
  real(kind=8)  :: tempb1(0:PDeg1,0:PDeg2), tempb2(0:PDeg1,0:PDeg2)
  integer:: e_flag
...
!                    (-a_vec(1)*Bq(2,1,DDK)+a_vec(2)*Bq(1,1,DDK))*(tempb2*B_tal2x)
      ...
      F_ns(i,j,:,:,DDK,l) = (GH(i)*Bq(2,2,DDK)-GH(j)*Bq(1,2,DDK))*(tempb1*B_tal1y)+&
                    (-GH(i)*Bq(2,1,DDK)+GH(j)*Bq(1,1,DDK))*(tempb2*B_tal2x)

      F_ns(i,j,:,:,DDK,l) = F_ns(i,j,:,:,DDK,l) - flx_F(i,j,:,:,DDK)-FS(i,j,:,:,DDK)
      F_ns(i,j,:,:,DDK,l) = F_ns(i,j,:,:,DDK,l) *A1(:,:,DDK)
  ...
  enddo
      F_s=0d0
!  deallocate( tempb1,tempb2 )
endsubroutine DG2D_flux_erk

!====================================================================
subroutine DG2D_flux_ark(l)
  use Legendre
  use MD2D_Grid
  use Metric_Var
  use NorVec_Var
...
!  real(kind=8),allocatable :: tempb1(:,:),tempb2(:,:)
  ...
  integer:: e_flag
  integer :: n,iupar(2), iterm
  integer :: it_hist(IGH*IGH*(PDeg1+1)*(PDeg2+1),3)
  real(kind=8)  :: rupar, tol(2), Fi(IGH*IGH*(PDeg1+1)*(PDeg2+1))
...
do DDK=1,TotNum_DM
iupar(1)=DDK
n= IGH*IGH*(PDeg1+1)*(PDeg2+1)
Fi=reshape(F_alt(1:IGH,1:IGH,0:PDeg1,0:PDeg2,DDK),(/ IGH*IGH*(PDeg1+1)*(PDeg2+1) /) )
call NSOLI(Fi,BGKimexS_nsoli,tol, ipari, rpari, Fi, it_hist, iterm, n, &
 iupar, rupar)
...
stop
    ...
    F_alt(1:IGH,1:IGH,0:PDeg1,0:PDeg2,DDK)=reshape(Fi, &
  (/ IGH,IGH,PDeg1+1,PDeg2+1  /))
...
subroutine DG_compute_error(error_L2,error_max,time)!,a_vec)
  ...
  real(kind=8), allocatable:: exa_solij(:,:,:,:,:)
  ...
  allocate( uhhij(1:IGH,1:IGH,0:PND1,0:PND2,1:TotNum_DM),&
     ...
     stat=ierr  )
  allocate( exa_solij(1:IGH,1:IGH,0:PND1,0:PND2,1:TotNum_DM)) 
  if (I_prob .ge. 2) then
  do DDK=1,TotNum_DM
   do ii=1,IGH
   do jj=1,IGH
...
enddo
  ...
  deallocate( uhhij,tempsum,temp,exa_solij )
return
