module RK_Var
  ...
  real(kind=8), allocatable:: RK_para_a(:),RK_para_b(:),RK_para_c(:)
  real(kind=8), allocatable:: const_a_E(:,:),const_a_I(:,:)
  real(kind=8), allocatable:: const_b(:),const_b_hat(:),const_c(:)
  real(kind=8)             :: time, time_loc 
  integer                  :: ks
  integer                  :: First_Comp, Last_Comp
  integer:: RK_Stage  
!  integer:: input(10)
...
contains 
  ...
  subroutine Init_SSPRK(rk,alpha)
    implicit none   
    
    integer:: rk
    integer:: i,j,k
    
    real(kind=8):: temp,alpha(1:rk)
    !
    alpha(1)=1;
    do i=1,rk
       do j=(i-1),1,-1
...
!============ coeffice of ARK4_6 ============
    ...
    const_b = (/ 82889d0/524892d0, 0d0, 15625d0/83664d0, 69875d0/102672d0, -2260d0/8211d0, 1d0/4d0 /)
