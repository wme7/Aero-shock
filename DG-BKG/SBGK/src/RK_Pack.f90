...
subroutine BGKimexL_nsoli(xcur, fcur, iupar, rupar)
...
real(kind=8), intent(in):: xcur(1:NV*nx_g*pp_g)
...
call Comp_ZTP_F(nnx,pp, IT, Rloc, Uloc, ETloc, Zloc, Tloc)
...
return
end subroutine BGKimexL_nsoli

! --------------------------------------------------------------------


  subroutine DG_flux_ark(nnx,pp,l,dx) !,c,b,A,F,u_t)
    use State_Var, ONLY: NV,F, F_s, F_ns, V, FS, BC_type,dx_g,dt_g
    use RK_Var
    use Legendre
!    use Nitsol_Var
    implicit none
...
real(kind=8), intent(in):: xcur(1:NV*pp_g)
real(kind=8), intent(out) :: fcur(1:NV*pp_g)
integer :: iupar(3) 
real(kind=8):: rupar(2)
...
integer :: K,m
real(kind=8):: FC(1:pp_g), FB(1:pp_g)
real(kind=8):: FS_loc(1:NV,1:pp_g)
real(kind=8):: vin(1:NV,1:pp_g)
real(kind=8):: ft_a(1:NV,1:pp_g)
real(kind=8):: FEQ_l(1:NV,1:pp_g)
real(kind=8):: Mtemp(NV,pp_g)
real(kind=8):: F_loc(1:NV,1:pp_g)

real(kind=8):: SR(1:pp_g), SU
real(kind=8):: SE,SAV
real(kind=8):: Rloc(1:pp_g), Uloc(1:pp_g)
real(kind=8):: Zloc(1:pp_g), Tloc(1:pp_g)
real(kind=8):: ETloc(1:pp_g)
logical isnan
...
call Comp_ZTP_S(nnx,pp, IT, Rloc, Uloc, ETloc, Zloc, Tloc)
!    do i=1,nnx
!        do m=1,pp
!            Tloc(i,m)    = 4d0*ETloc(i,m)/Rloc(i,m) - 2d0*Uloc(i,m)**2
!            Zloc(i,m)    = Rloc(i,m) / sqrt(pi* Tloc(i,m))
...
!call EQUILIBRIUM_F(nnx,pp)
    do K=1,NV
     do m=1,pp
    FEQ_l(K,m)   = 1d0/((exp( (V(K)-Uloc(m))**2 /Tloc(m))/Zloc(m)) + dble(IT) )

    if (FEQ_l(K,m) .gt. 1d20) then
    write(6,*) "Infty",K,m
    stop
    endif
     END DO
...
!call Compute_Source_F(nnx,pp,dx,vin,FS_loc)
        do K = 1,NV
         ...
         FC=matmul(FC,Pleg)-FB
          do j=0,p
           FS_loc(K,j+1)=sum(FC*Pleg(j+1,:)*wl)*dx/2d0/VIS(DDK,j+1)
          enddo
        enddo
...
ft_a= vin - F(1:NV,DDK,1:pp_g) - gamma_ark*dt*FS_loc

fcur=reshape(ft_a, (/ NV*pp /) )
!write(6,*) "fcur", maxval(abs(fcur))
...
!------------------------------------------------------------------------------
  ...
  subroutine DG_flux_arks(nnx,pp,l,dx) !,c,b,A,F,u_t)
    use State_Var, ONLY: NV,F, F_s, F_ns, V, FS, BC_type,dx_g,dt_g
    use RK_Var
    use Legendre
!    use Nitsol_Var
...
real(kind=8):: rupar(2)
  ...
  real(kind=8):: Fi(NV*pp),fout(NV*pp)
  real(kind=8):: fnrm, ftol,gamma_ark,dtt
  real(kind=8)::rpari, tol(2),it_hist(NV*nnx*pp,3)
  integer:: ipari(4)
...
Fi=reshape(F(1:NV,i,1:pp),(/ NV*pp /) )

call NSOLI(Fi,BGKimexS_nsoli,tol, ipari, rpari, Fi, it_hist, ierr, NV*pp, &
 iupar, rupar)
...
stop
    ...
    F(1:NV,i,1:pp)=reshape(Fi,(/ NV,pp /))
