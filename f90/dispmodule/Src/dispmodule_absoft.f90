...
! SPECIAL VERSION THAT WORKS WITH THE ABSOFT AF95 COMPILER, VERSION 10.1
!
...
CONTAINS
  ...
  subroutine get_SE(SE, title, shapex, fmt, advance, lbound, seperator, style, trim, unit, orient, zeroas, digmax)
    ! Get the settings from the optional parameters fmt...zeroas into the structure SE.
...
CONTAINS
  ...
  subroutine find_editdesc_dint(x, SE, wid, nbl)
    ...
    integer ww
    !
    if (SE % w == 0) then  ! Edit descriptor 'I0' specified
      xp = maxval(x)
      xm = minval(x)
    ...
    elseif (SE % w < 0) then ! No edit descriptor specified; obtain max-width of x
      if (size(x) == 0) then
        SE % ed = '()'
        SE % w = 0
        wid = 0
  ...
  function tostring_s_dint(x) result(st)
    ! Scalar to string
    integer(dint), intent(in)                      :: x
    character(len_f_dint((/x/), 1, tosset % ifmt)) :: st
  ...
  function tostring_sf_dint(x, fmt) result(st)
    ...
    integer(dint),intent(in)             :: x
    character(*), intent(in)             :: fmt
    character(len_f_dint((/x/), 1, fmt)) :: st
  ...
  function tostring_dint(x) result(st)
    ! Vector to string
    integer(dint), intent(in) :: x(:)
    character(len_f_dint(x, size(x), tosset % ifmt)) :: st
    st = tostring_f_dint(x, tosset % ifmt)
  ...
  function tostring_f_dint(x, fmt) result(st)
    ...
    integer(dint), intent(in)                 :: x(:)
    character(*), intent(in)                  :: fmt
    character(len_f_dint(x,  size(x), fmt))   :: st
    character(widthmax_dint(x, size(x), fmt)) :: sa(size(x))
    integer                                   :: w, d
    logical                                   :: gedit
  ...
  pure function len_f_dint(x, ix, fmt) result(wtot)
    ! Total width of tostring representation of x
    integer, intent(in)                       :: ix     ! This parameter is needed for Absoft's compiler
    integer(dint), intent(in)                 :: x(ix)  ! (with other compilers x can be dimensioned with x(:))
    character(*), intent(in)                  :: fmt
    character(widthmax_dint(x, size(x), fmt)) :: sa(size(x))
    integer                                   :: wtot, w, d
    logical                                   :: gedit
  ...
  pure function widthmax_dint(x, ix, fmt) result(w)
    ! Maximum width of string representation of an element in x
    integer, intent(in)        :: ix
    integer(dint), intent(in)  :: x(ix)  ! ix is a parameter to avoid bug in Absoft f95
    character(*), intent(in)   :: fmt
  ...
  function tostring_s_sngl(x) result(st)
    ! Scalar to string
    real(sngl), intent(in)                         :: x
    character(len_f_sngl((/x/), 1, tosset % rfmt)) :: st
  ...
  function tostring_sf_sngl(x, fmt) result(st)
    ...
    real(sngl),   intent(in)             :: x
    character(*), intent(in)             :: fmt
    character(len_f_sngl((/x/), 1, fmt)) :: st
  ...
  function tostring_sngl(x) result(st)
    ! Vector to string
    real(sngl), intent(in)                           :: x(:)
    character(len_f_sngl(x, size(x), tosset % rfmt)) :: st
    st = tostring_f_sngl(x, tosset % rfmt)
  ...
  function tostring_f_sngl(x, fmt) result(st)
    ...
    real(sngl)    ,       intent(in)          :: x(:)
    character(*),         intent(in)          :: fmt
    character(len_f_sngl(x, size(x), fmt))    :: st
    character(widthmax_sngl(x, size(x), fmt)) :: sa(size(x))
  ...
  pure function len_f_sngl(x, ix, fmt) result(wtot)
    ! Total length of returned string, vector s
    integer, intent(in)                       :: ix     ! ix is needed to avoid bug in Absoft f95
    real(sngl), intent(in)                    :: x(ix)
    character(*), intent(in)                  :: fmt
    character(widthmax_sngl(x, size(x), fmt)) :: sa(size(x))
    integer                                   :: wtot, w, d, ww
    logical                                   :: gedit
  ...
  pure function widthmax_sngl(x, ix, fmt) result(w)
  ...
  pure function widthmax_re_cplx(x, ix, fmt) result(w) !Needed because of bug in Absoft's compiler
    integer, intent(in)       :: ix
    complex(sngl), intent(in) :: x(ix)
    character(*), intent(in)  :: fmt
    integer :: w
    real(sngl) :: re_x(size(x))
    re_x = real(x)
    w = widthmax_sngl(re_x, size(x), fmt)
  end function widthmax_re_cplx
    
  pure function widthmax_im_cplx(x, ix, fmt) result(w) !Needed because of bug in Absoft's compiler
    integer, intent(in)       :: ix
    complex(sngl), intent(in) :: x(ix)
    character(*), intent(in)  :: fmt
    integer                   :: w
    real(sngl)                :: im_x(size(x))
    im_x = aimag(x)
    w = widthmax_sngl(im_x, size(x), fmt)
  end function widthmax_im_cplx
  ...
  function tostring_cplx(x) result(st)
    complex(sngl), intent(in)                        :: x(:)
    character(len_f_cplx(x, size(x), tosset % rfmt)) :: st
    st = tostring_f_cplx(x, tosset % rfmt)
  ...
  function tostring_f_cplx(x, fmt) result(st)
    complex(sngl),  intent(in)                   :: x(:)
    character(*),   intent(in)                   :: fmt
    character(len_f_cplx(x, size(x), fmt))       :: st
    character(widthmax_re_cplx(x, size(x), fmt)) :: sar(size(x)) ! Here widthmax_sngl(real(x)...) could
    character(widthmax_im_cplx(x, size(x), fmt)) :: sai(size(x)) ! be used except with Absoft's compiler
    character(1)                                 :: sgn(size(x))
    integer                                      :: w, d, wr, wi, i
    logical                                      :: gedit
    character(nnblk(fmt)+8)                      :: fmt1  !(5 for readfmt and 3 for replace_w)
  ...
  pure function len_s_cplx(x, fmt) result(wtot)
    ...
    call readfmt(fmt, fmt1, w, d, gedit)
    if (w < 0) then; wtot = len(errormsg); return; endif
    wtot = len_f_sngl((/real(x)/), 1, fmt) + len_f_sngl((/abs(aimag(x))/), 1, fmt) + 4
  end function len_s_cplx

  pure function len_f_cplx(x, ix, fmt) result(wtot)
    integer, intent(in)       :: ix    ! ix is needed to avoid bug in Absoft f95
    complex(sngl), intent(in) :: x(ix)
    character(*), intent(in)  :: fmt
    integer                   :: wtot, w, d
    ...
    call readfmt(fmt, fmt1, w, d, gedit)
    if (w < 0) then; wtot = len(errormsg); return; endif
    wtot = len_f_sngl(real(x), ix, fmt) + len_f_sngl(abs(aimag(x)), ix, fmt) + size(x)*4 - (size(x) - 1)*(tosset % seplen)
  ...
  function tostring_s_dble(x) result(st)
    ! Scalar to string
    real(dble), intent(in)                         :: x
    character(len_f_dble((/x/), 1, tosset % rfmt)) :: st
  ...
  function tostring_sf_dble(x, fmt) result(st)
    ...
    real(dble),   intent(in)             :: x
    character(*), intent(in)             :: fmt
    character(len_f_dble((/x/), 1, fmt)) :: st
  ...
  function tostring_dble(x) result(st)
    ! Vector to string
    real(dble), intent(in)                           :: x(:)
    character(len_f_dble(x, size(x), tosset % rfmt)) :: st
    st = tostring_f_dble(x, tosset % rfmt)
  ...
  function tostring_f_dble(x, fmt) result(st)
    ...
    real(dble)    ,       intent(in)          :: x(:)
    character(*),         intent(in)          :: fmt
    character(len_f_dble(x, size(x), fmt))    :: st
    character(widthmax_dble(x, size(x), fmt)) :: sa(size(x))
    character(nnblk(fmt)+8)                   :: fmt1  !(5 for readfmt and 3 for replace_w)
    integer                                   :: w, d, ww
    logical                                   :: gedit!, xfinite(size(x))
    call readfmt(fmt, fmt1, w, d, gedit)
    if (w < 0) then
      st = errormsg
      return
    elseif (w == 0) then
      !xfinite = x == x .and. x <= huge(x) .and. x >= -huge(x)  ! neither NaN, Inf nor -Inf
      ww = maxw_dble(x, d)
      !ww = find_f0format_width_dble(minval(x, mask=xfinite), maxval(x, mask=xfinite), d) ! ww is maxwidth, later trimmed
      call replace_w(fmt1, ww)      
    endif
    write(sa, fmt1) x
    call trim_real(sa, gedit, w)
  ...
  pure function len_f_dble(x, ix, fmt) result(wtot)
    ! Total length of returned string, vector s
    integer, intent(in)                       :: ix     ! ix is needed to avoid bug in Absoft f95
    real(dble), intent(in)                    :: x(ix)
    character(*), intent(in)                  :: fmt
    character(widthmax_dble(x, size(x), fmt)) :: sa(size(x))
    integer                                   :: wtot, w, d, ww
    logical                                   :: gedit!, xfinite(size(x))
  ...
  pure function widthmax_dble(x, ix, fmt) result(w)
    ! Maximum width of an element of x
    integer, intent(in)      :: ix     ! ix is needed to avoid bug in Absoft f95
    real(dble), intent(in)   :: x(ix)
    character(*), intent(in) :: fmt
    ...
    elseif (w == 0) then
      !xfinite = x == x .and. x <= huge(x) .and. x >= -huge(x)  ! neither NaN, Inf nor -Inf
      !w = find_f0format_width_dble(minval(x, mask=xfinite), maxval(x, mask=xfinite), d)
      w = maxw_dble(x, d)
    endif
  ...
  pure function widthmax_re_cpld(x, ix, fmt) result(w) !Needed because of bug in Absoft's compiler
    integer, intent(in)       :: ix
    complex(dble), intent(in) :: x(ix)
    character(*), intent(in)  :: fmt
    integer :: w
    real(dble) :: re_x(size(x))
    re_x = real(x)
    w = widthmax_dble(re_x, size(x), fmt)
  end function widthmax_re_cpld
    
  pure function widthmax_im_cpld(x, ix, fmt) result(w) !Needed because of bug in Absoft's compiler
    integer, intent(in)       :: ix
    complex(dble), intent(in) :: x(ix)
    character(*), intent(in)  :: fmt
    integer                   :: w
    real(dble)                :: im_x(size(x))
    im_x = aimag(x)
    w = widthmax_dble(im_x, size(x), fmt)
  end function widthmax_im_cpld
  ...
  function tostring_cpld(x) result(st)
    complex(dble), intent(in)                        :: x(:)
    character(len_f_cpld(x, size(x), tosset % rfmt)) :: st
    st = tostring_f_cpld(x, tosset % rfmt)
  ...
  function tostring_f_cpld(x, fmt) result(st)
    complex(dble),  intent(in)                   :: x(:)
    character(*),   intent(in)                   :: fmt
    character(len_f_cpld(x, size(x), fmt))       :: st
    character(widthmax_re_cpld(x, size(x), fmt)) :: sar(size(x)) ! Here widthmax_dble(real(x)...) could
    character(widthmax_im_cpld(x, size(x), fmt)) :: sai(size(x)) ! be used except with Absoft's compiler
    character(1)                                 :: sgn(size(x))
    integer                                      :: w, d, wr, wi, i
    logical                                      :: gedit!, xrefinite(size(x)), ximfinite(size(x))
    character(nnblk(fmt)+8)                      :: fmt1  !(5 for readfmt and 3 for replace_w)
    ...
    xim = aimag(x)
    h = huge(h)
    !xrefinite = xre == xre .and. xre >= -h .and. xre <= h
    !ximfinite = xim == xim .and. xim >= -h .and. xim <= h
    if (w < 0) then
      st = errormsg
      return
    elseif (w == 0) then
      !wr = find_f0format_width_dble(minval(xre, mask=xrefinite), maxval(xre, mask=xrefinite), d)
      !wi = find_f0format_width_dble(0.0_dble, maxval(abs(xim), mask=ximfinite), d)
      wr = maxw_dble(xre, d)
      wi = maxw_dble(xim, d)
      call replace_w(fmt1, max(wr, wi))
    endif
  ...
  pure function len_s_cpld(x, fmt) result(wtot)
    ...
    call readfmt(fmt, fmt1, w, d, gedit)
    if (w < 0) then; wtot = len(errormsg); return; endif
    wtot = len_f_dble((/real(x)/), 1, fmt) + len_f_dble((/abs(aimag(x))/), 1, fmt) + 4
  end function len_s_cpld

  pure function len_f_cpld(x, ix, fmt) result(wtot)
    integer, intent(in)       :: ix    ! ix is needed to avoid bug in Absoft f95
    complex(dble), intent(in) :: x(ix)
    character(*), intent(in)  :: fmt
    integer                   :: wtot, w, d
    ...
    call readfmt(fmt, fmt1, w, d, gedit)
    if (w < 0) then; wtot = len(errormsg); return; endif
    wtot = len_f_dble(real(x), ix, fmt) + len_f_dble(abs(aimag(x)), ix, fmt) + size(x)*4 - (size(x) - 1)*(tosset % seplen)
  ...
  function tostring_sf_dlog(x, fmt) result(st)
    logical(dlog),intent(in)        :: x
    character(*), intent(in)        :: fmt
    character(len_f_dlog((/x/), 1, fmt)) :: st
  ...
  function tostring_f_dlog(x, fmt) result(st)
    logical(dlog), intent(in)              :: x(:)
    character(*), intent(in)               :: fmt
    character(len_f_dlog(x, size(x), fmt)) :: st
  ...
  pure function len_f_dlog(x, ix, fmt) result(wtot)
    integer, intent(in)        :: ix
    logical(dlog), intent(in)  :: x(ix)
    character(*), intent(in)   :: fmt
    integer                    :: wtot, w, d
    logical                    :: gedit
    character(nnblk(fmt)+5)    :: fmt1
